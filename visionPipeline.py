...
SPANNEISEN_MARKER_OFFSET_Z_M = 0.024 # 20mm dicke Spanneisen + 4mm dicke Marker (Kleber + Aluplatte)
z_spanneisen_marker = 0.0
...
def detect_aruco(charuco_frame, valid_mask=None):
    ...
    for i, marker_id in enumerate(aruco_ids.flatten()):
        ...
        p0_3d = pixel_to_3dplane(
            ...
            float(corners[0, 1]),
            z_spanneisen_marker,
            H_image_to_plane_override=(
                H_image_to_spanneisen_plane
            )
        )
...
def getWorkPlaneHomography(z_plane_work=None):
    if z_plane_work is None:
        z_plane_work = z_weld_surface

    z_plane_work = float(z_plane_work)
    ...
    r3_new = t_work_to_camera + z_plane_work * r3
...
def pixel_to_3dplane(u, v, z_h, H_image_to_plane_override=None):
    global H_image_to_plane

    H_inverse = (
        H_image_to_plane
        if H_image_to_plane_override is None
        else H_image_to_plane_override
    )
    ...
    if H_inverse is None:
        raise ValueError(
            "Inverse Homographie nicht initialisiert"
        )

    # Umrechnen der Bildkoordinate in Raumkoordinate
    X_plane = H_inverse @ x_pixel
...
def plane_to_pixel(point_3d, expected_z, H_plane_to_image_override=None):

    H_forward = (
        H_plane_to_image
        if H_plane_to_image_override is None
        else H_plane_to_image_override
    )

    if H_forward is None:
        raise ValueError("Vorwärtshomographie ist nicht initialisiert. Bitte zuerst die Homographie berechnen.")
        ...
        for u_norm, v_norm in poly_norm:
            ...
            try:
                u_px, v_px = plane_to_pixel(
                    np.array([world_x, world_y, z_spanneisen_marker], dtype=np.float64),
                    z_spanneisen_marker,
                    H_plane_to_image_override=(
                        H_spanneisen_plane_to_image
                    )
                )
...
def runVisionPipeline(frame, camera_info, calibration_file, plate_mm, pipeline_mode, assisted_polygon_pixels=None):
    ...
    global H_spanneisen_plane_to_image, H_image_to_spanneisen_plane
    global plate_thickness, plate_thickness_m
    global z_weld_surface, z_spanneisen_marker
    global spanneisen_positions
    ...
    z_spanneisen_marker = (
        z_weld_surface + SPANNEISEN_MARKER_OFFSET_Z_M
    )
    ...
    # Schweißebene - Offset des Werkobjektkoordinatensystems in die Homographie einbeziehen (inkl. Rotation und Translation des Koordinatensystems (auch in Z))
    H_plane_to_image, H_image_to_plane = getWorkPlaneHomography()

    # Spanneisen-Ebene
    H_spanneisen_plane_to_image, H_image_to_spanneisen_plane = getWorkPlaneHomography(z_spanneisen_marker)
